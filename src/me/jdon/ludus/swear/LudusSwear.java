...
public class LudusSwear extends JavaPlugin implements Listener{
	...
	// lowest event priority to stop lagg
	 ...
	 public void chat(final PlayerChatEvent ev) {
         ...
         for (String s : swears){
        	 for (String e : exception){
            	 if(final1.contains(e))return;
            	 if(me15.contains(e))return;
            	 if(me14.contains(e))return;
            	 if(final2.contains(e))return;
             }
	 ...
	 public String swearjar(Player p, String m){
		Double amount = this.getConfig().getDouble("Fine");
